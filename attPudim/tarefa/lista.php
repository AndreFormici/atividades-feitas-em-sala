<?php 
  ...
  $email = $_POST['email'];
  $pasta = "imagens/";
  $arquivo = $pasta . basename($_FILES['imagemUpload']['name']);
  $tipo = strtolower(pathinfo($arquivo, PATHINFO_EXTENSION));
  if(move_uploaded_file($_FILES['imagemUpload']['tmp_name'], $arquivo)){   
    if($nome != null && $email != null){
      $sql = "INSERT INTO attcadastrobanco (nome, e-mail, imagem) VALUES ('$nome', '$email', '$arquivo')";
      $texto = "";
      if($conn->query($sql) == TRUE){
        $texto = "Dado inserido com sucesso";
      }
      else{
        $texto = "Error: " . $conn->error;
      }
    }
  }
  $list = "SELECT * FROM attcadastro";
